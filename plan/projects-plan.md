...
**Status:** 2 of 10 milestones complete; M3 implementation complete, live verification pending; M3.5 Exposé confirmed, extended display verification pending
...
User requested task 3.31: let Project buttons determine the minimum switcher width and use two rows. `TilesView.swift` may prepare the header before sizing, reserve its rows during automatic sizing, and keep the widened tile viewport centered and unclipped.

User requested Desktop destination badges and a previous-Desktop return action. Task 3.29 adds badge layout in `TileView.swift`, return selection in `SwitcherSession.swift`, `App.swift`, `Windows.swift` and `TilesView.swift`, and configured system-shortcut fallback in `SpacesList.swift`. These navigation controls also work with Projects disabled. Task 3.30 adds local Command-Z routing in `KeyboardEventsTestable.swift` and its test-target stub in `Mocks.swift`.
...
> Note: User-requested M3.5 also adds per-Space label windows under `src/spaces/`, with lifecycle integration under `src/projects/`. These windows use the existing Projects enablement guard and do not change the Spaces switcher's controls.
...
- [x] 2.7 — Menu bar, *depends on 2.6*: one block in `Menubar.initialize()` adding "Name this Desktop…" (always) and, **only when enabled**, a "Projects" submenu with "New Project from this Window…", "Add this Window to Project ▸", "Rename Project ▸", "Delete Project ▸". One call in `menuWillOpen` delegates to `ProjectsMenu.refresh(_:)`, which lives in the projects folder and owns the dynamic submenus and hidden state. Files: `src/Menubar.swift`, `src/projects/ProjectsMenu.swift`, `alt-tab-macos.xcodeproj/project.pbxproj`

> Note: Debug build passed. Real NSMenu smoke checks with model/prompt fixtures passed disabled visibility, enabled actions, focused-window membership, dynamic names, deletion and cancelled creation. Live AltTabDebug persisted automatic names for all 13 Desktops. The CUA tool cannot expose the menu-bar icon, so the physical menu/prompt walkthrough remains pending; no extra production test hook was added.
- [x] 2.8 — Regenerate the source strings. Files: `resources/l10n/Localizable.strings`

> Note: Debug build and all 1,196 tests passed. ensure_generated_files_are_up_to_date.sh exited 0. Both resolver triads are clean; protected upstream tests remain untouched. The live Spaces panel was captured before extraction and retains all 13 Desktop tiles. Physical menu/prompt walkthrough remains pending as recorded in 2.7.
...
**Commit:** per task
**Status:** implementation complete (28 of 28 tasks); live verification pending
...
> Note: Debug build passed. Live before/after screenshots show the same 13 tiles, two-row grid, dimensions, typography, icons and highlight. Only Desktop 1/2 MRU order changed across relaunch. Escape still dismisses Spaces. Existing SpacesOrderResolver tests are unchanged; full suite remains at 1,196 passing tests.
...
- [x] 3.17 — User-requested Desktop-to-Project linking in “Name this Desktop…”: checkbox and new/existing Project picker, persistent one-to-one links, automatic membership for existing and future windows on linked Desktops (explicitly approved), and linked Project selection when entering its Desktop if the task 3.18 setting is enabled. Manual cross-Desktop members remain allowed. Unlinking retains memberships; deleting a Project clears links. Files stay in `src/projects/`, project registration, generated strings and docs. General unlinked-Project M4 lifecycle remains separate. Validation: Debug build; separate-process registry smoke checks for persistence, existing/future membership, unique links, unlink retention and deletion cleanup; production dialog inspected in an isolated AppKit fixture with inline documentation screenshot.
- [x] 3.18 — User-requested default, revised 2026-09-08: “Switch active Project when switching Desktops” defaults on. Desktop changes, including ⌘F1/⌘F2, select the destination’s linked Project or normal Desktop scope. Startup also follows the current Desktop instead of restoring a stale saved Project. Turning the setting off preserves a custom Project across Desktop changes and restores it at startup. Linked membership capture is independent of this setting. Files: `src/projects/Projects.swift`, `ProjectsPreferences.swift`, `ProjectsSheet.swift`, generated strings and docs.

> Validation, 2026-09-08: Debug build and all 1,284 tests pass. The rebuilt app’s settings show Desktop following on with no saved override. A live Spaces-switcher round trip selected “agent-skills” on Desktop 2 and “alt-tab” on Desktop 1, confirmed by active-Project logs and persisted IDs. Direct ⌘F1/⌘F2 automation was unavailable because the computer-use tool cannot send the system shortcut’s Fn modifier. Signing identity, Team ID and bundle ID are unchanged.
...
- [x] 3.27 — User-requested numbering correction: 1–9 then 0, later entries named and click-only. Route real left-button gestures through Project controls, matching down/up targets and using event coordinates; pass popover background events to AppKit. Files: Project header/number resolver triad, approved CursorEvents hook, docs and plan. Validation: Debug build and 1,216 tests pass; live strip/grid selection, click-only Project selection, press/release cancellation, and digit 0/1 selection verified. Updated screenshot saved inline in docs. App rebuilt/restarted and Focus-on-release restored.
- [x] 3.28 — Fix Project pointer hit areas after a physical Lilly click was incorrectly routed to the Desktop: intersect view bounds with visibleRect, which can span the whole non-clipping switcher. Add a ProjectPointerResolver triad using the captured coordinates and popover clipping cases. Files: Project header, new resolver triad, test/app registration, docs and plan. Validation: Debug build and all 1,219 tests pass, including the captured physical-click regression and clipping cases. App rebuilt/restarted; temporary coordinate dump removed and Focus-on-release restored. User confirmed physical clicks work on the corrected build.
- [x] 3.29 — Mark windows that switch Desktops with a destination badge. Remember the previous Desktop, Project and focused window; expose Back with normal release/Return confirmation. Task 3.30 defines its current cycling and undo behavior; search retains its existing navigation. Returning restores the saved Project and supports reversing the trip; stale/deleted destinations are guarded. Empty Desktops honor configured system shortcuts, including Command/Fn bindings. Files: DesktopNavigationResolver triad, DesktopNavigation, DesktopDestinationBadge, Project header/observer and the integration files listed above. Validation: Debug build and all 1,299 tests pass, including 15 new resolver scenarios; targeted spec/test audit is clean. Native thumbnail badges and accessibility destination help verified. A live cross-Desktop window switch activated the destination Project, and selecting Back with the previous-window action plus Return restored Desktop 1 and its alt-tab Project. Modifier-only Shift and held-Command release still require a physical keyboard check because the automation API sends complete key chords. App restarted; Focus-on-release and Space labels restored.
- [x] 3.30 — User-requested navigation correction: Back is a regular cycle stop between the last and first visible windows. Shift retains ordinary backward cycling; cycling continues out of Back in either direction. Command-Z returns directly while the window switcher is open and preserves text undo during search editing. Repeat stops at the extended list's ends; no-wrap spatial navigation retains its existing window limits. Updated resolver tests/specs, adapter, keyboard hook/stub, button help, strings and docs. Validation: Debug build and all 1,307 tests pass; 23 DesktopNavigationResolver scenarios match their tests. Live checks verified backward window navigation, cycling through Back in both directions, and Command-Z returning from Desktop 2 to Desktop 1 with the saved alt-tab Project while a window was selected. Command-Z during search left the Desktop unchanged. The rebuilt app is running with Focus-on-release and Space labels restored.
- [x] 3.31 — User-requested wider Project navigation with two rows. The Project strip determines a minimum panel width independently of window count, capped at 90% of the usable screen width. Measured buttons keep their order, wrap to two rows, and reserve All Projects for overflow. Automatic thumbnail sizing reserves the new header height; widened tile viewports stay centered and unclipped in both writing directions. Files: ProjectStripLayoutResolver triad, ProjectContextHeader, TilesView, registration and docs. Validation: Debug build and all 1,316 tests pass; nine ProjectStripLayoutResolver scenarios match their tests. Native Thumbnails checks show all 15 Projects across two balanced rows in a 1,323-point-wide panel, including a one-window ui Project and an empty Blank Project. Second-row selection works. The app is rebuilt and running; the original Thumbnails appearance, Focus-on-release, alt-tab Project and Space labels are restored.
...
### Milestone 3.5 — Space label windows

**Objective:** With Projects enabled, “Show Space Labels” creates a closable window on every Space, displaying its label and number in large white text on solid black. Each window uses **50% of its screen’s usable width**, with the number and name in one bold line whose font grows to fill that width. Height follows the text to form a compact banner. Show or Bring to Front raises the set once; clicking elsewhere sends it behind ordinary windows. Native and bulk minimize actions are available, and Bring to Front restores minimized labels for Mission Control / Exposé. A native close button dismisses one label; “Close All Space Labels” removes the set. Labels start hidden and reopen only on request.
**Commit:** per task
**Status:** implementation complete; user confirmed Exposé visibility; fullscreen and multiple-display live verification pending

- [~] M3.5.1 — Prove per-Space placement and Mission Control / Exposé visibility with a minimal AppKit window before building the feature. Verify inactive and empty Desktops, fullscreen Spaces, and multiple displays. Each window belongs to its own Space; a single window replicated across all Spaces cannot show distinct labels. Record the required window level, collection behavior, Space-assignment mechanism, and screenshots. Ordinary floating-window behavior alone does not satisfy the milestone. If macOS prevents the requested visibility or an additional upstream seam is needed, record the evidence and raise the blocker. Files: `src/spaces/SpaceLabelWindow.swift`, `alt-tab-macos.xcodeproj/project.pbxproj`, `docs/projects/`, `plan/projects-plan.md`
- [x] M3.5.2 — Add a pure `SpaceLabelResolver` triad, *depends on M3.5.1*: key labels by Space UUID; resolve the saved explicit name, then sticky automatic name, then an unnamed fallback. Show the current Desktop number separately from the label, never a Project shortcut number. Fullscreen Spaces currently expose `desktopNumber == 0`; use an explicitly labeled Space ordinal for them, never “Desktop 0”. Cover rename/clear, duplicate names, long names, empty Spaces, reorder/renumber, fullscreen, and changed session IDs with stable UUIDs. Files: `src/spaces/SpaceLabelResolver.swift`, `src/spaces/SpaceLabelResolverSpecs.md`, `src/spaces/SpaceLabelResolverTests.swift`, `alt-tab-macos.xcodeproj/project.pbxproj`
- [x] M3.5.3 — Build the label window, *depends on M3.5.1 and M3.5.2*: large, high-contrast name and number in a fixed 500 × 200 px frame, consistently positioned near the top center of its display. Account for AppKit points versus backing pixels and verify the requested size on Retina and non-Retina displays. Keep the number readable when a long name wraps or truncates. Windows are non-activating and click-through; they never become key, steal focus, or appear as selectable AltTab windows or Project members. Files: `src/spaces/SpaceLabelWindow.swift`, `resources/l10n/Localizable.strings`
- [x] M3.5.4 — Manage one reusable window per Space UUID, *depends on M3.5.3*, using existing startup, Projects enablement, naming, Space, and display notifications. Create/remove windows as Spaces appear/disappear; update names, numbers, placement, and backing scale when they change; close all label windows when Projects is disabled. Include fullscreen Spaces independently of switcher filtering. Use observers, not polling. Keep AppKit on main, reuse existing off-main CGS scheduling where applicable, and defer bookkeeping beyond latency-critical visible work. Files: `src/spaces/SpaceLabelWindows.swift`, `src/projects/Projects.swift`, `src/projects/ProjectsSheet.swift`, `alt-tab-macos.xcodeproj/project.pbxproj`
- [~] M3.5.5 — Complete the live walkthrough and document results, *depends on M3.5.4*. Regenerate strings, audit the resolver triad, and capture ordinary Desktop and Mission Control / Exposé views showing distinct labels simultaneously. Files: `resources/l10n/Localizable.strings`, `docs/projects/`, `plan/projects-plan.md`

> Note: 2026-09-08 — Debug build and all 1,267 tests pass, including 17 new SpaceLabelResolver scenarios with a clean spec/test and comment audit. The generated-file check passes in an isolated staged copy so unrelated working changes do not affect its result. A native production-source fixture verifies 500 × 200 backing pixels on this 2× display, non-key/non-main click-through behavior, and successful assignment to all 14 current Desktops. A production-source lifecycle fixture verifies reuse, rename, removal, session-ID reassignment, re-enable, and disabling during pending assignment or topology reads. The rebuilt AltTabDebug is running: WindowServer reports 14 distinct single-Space memberships at floating level 3, alpha 1, with 250 × 100 point frames; none is in AltTab’s tracked window list. The temporary colored probe was stopped at the user’s request.

> Note: Mission Control automation times out, so its Space-preview visibility/readability is not claimed verified. This session has one 2× display and no fullscreen Spaces; live fullscreen, non-Retina, display reconnect and separate-Spaces configuration checks remain pending. The resolver tests cover their topology/numbering/geometry inputs. See `docs/projects/phase3_5-space-label.md` for the native render. No additional upstream integration files or signing identities were changed.

> Note: 2026-09-08 user corrections supersede the original 500 × 200 pixel, automatic-startup, and click-through requirements in M3.5.3–M3.5.4. Labels are now managed, titled NSWindows with a native close button. Their accessibility subrole keeps them out of AltTab’s ordinary window list. Show/Close All commands live in `ProjectsMenu.swift` and `ProjectsSheet.swift`; no additional upstream seam is needed. Closed UUIDs stay suppressed across refreshes until another explicit Show request, and restarting or disabling Projects ends the label session.

- [x] M3.5.6 — User-requested Exposé and dismissal correction: use managed windows; add native individual close, Show Space Labels and Close All Space Labels actions; stop automatic creation; test individual-close suppression, explicit recreation and cancellation during pending work. Files: SpaceLabelWindow, SpaceLabelWindows, SpaceLabelResolver triad, ProjectsMenu, ProjectsSheet, strings and docs. Live checks: zero labels at startup, Show creates 14, close button leaves 13, Close All leaves zero, Show recreates 14. User confirmed the revised labels appear in Exposé.
- [x] M3.5.7 — User-requested highest-contrast labels: bold white text on opaque black, using half the usable screen width. The user clarified that height can be short: number and name now share one line, the font fits the native text cell’s available width, and height follows the text. Very long names truncate at a readable minimum with the leading number preserved; the full title stays accessible. Geometry follows the owning screen and rounds to AppKit’s whole points to avoid repeated resize calls. Files: SpaceLabelWindow, SpaceLabelResolver triad, docs and plan. Native geometry, font-fit, Unicode, long-name and close checks pass. On the current 1470 × 923 point usable screen, “3 · Research” renders in a 735 × 189 point window at about 125 point type. Debug build and all 1,275 tests pass, including 25 resolver scenarios; the generated-file check and targeted triad/comment audits pass. The rebuilt app is running with 14 visible, half-width banners assigned to distinct Spaces, and its native UI shows the full-width text and close button.

- [x] M3.5.8 — User-requested temporary front ordering and minimization: use the normal window level; add Bring Space Labels to Front and Minimize All Space Labels in the menu and settings; enable the native yellow minimize button. Clicks outside labels and activation of another app send the set to the back without closing it. Bring restores minimized windows while preserving individual close decisions; Show also recreates closed labels. Deferred ordering requests carry a revision so a later Show, Bring, Minimize or Close cancels stale work, and bulk operations yield between windows. Routine refreshes do not raise or restore windows. Files: SpaceLabelWindow, SpaceLabelWindows, SpaceLabelResolver triad, ProjectsMenu, ProjectsSheet, strings and docs. Validation: 1,283 tests, 33 paired resolver scenarios, production-source lifecycle checks for back/raise/minimize/restore and cancellation, plus native geometry and window-control checks. Live Minimize All and Bring restored the same 14 IDs on their original Spaces at normal level; native settings buttons are visible. The computer-use tool reports that the Mac is locked. Reloading the final build with interruptible batches and physical click-away verification are pending an unlock; the preceding build’s minimize/restore controls were verified live.

> Note: The current display’s Desktop Exposé check is confirmed by the user. Live fullscreen Spaces, multiple displays, display reconnect and separate-Spaces configurations remain the unverified part of M3.5.1/M3.5.5. The original floating-overlay check above did not satisfy Exposé visibility and is superseded by M3.5.6. The updated native render is in `docs/projects/phase3_5-space-label.md`.

**Verification:**
1. Build using `ai/build.sh` commands; tests, `audit-specs-tests`, generated-file check, and upstream touch-budget diff check pass.
2. Show Space Labels creates one window per Space, each half its screen’s usable width, with a compact height and width-fitted bold white name/number on black. Bring raises them once; clicking elsewhere sends them behind ordinary windows. Minimize All hides the set in the Dock and Bring restores it without changing Space memberships.
3. Enter Mission Control / Exposé: every Space preview retains its own readable label, including inactive, empty, and fullscreen Spaces. Check multiple displays and “Displays have separate Spaces” configurations; record any unsupported behavior as a blocker, not a passing result.
4. Rename, clear a name, reorder/add/remove Spaces, reconnect a display, and relaunch AltTab: labels update without duplicates or stale assignments.
5. The native close button dismisses one label and refreshes do not recreate it. Close All removes the set; Show explicitly recreates it. Labels start hidden after restart or re-enabling Projects. Creation does not activate AltTab; intentional clicks can focus a label. Labels remain absent from switcher entries and Project membership.
6. No polling or repeated window creation while idle; `MainThreadStall.step()` checks confirm label maintenance does not delay summon, cycling, search, or dismissal.
...
User explicitly authorized this milestone after the reboot test. Milestones 4–7 remain unstarted. Exact app/title matching supersedes the proposed prefix/substring matching below to avoid accidental assignments.
...
- [x] 8.10 — Make historical Safari/Chrome titles and URL rows clickable when a saved HTTP(S) URL exists. Open through the original browser off-main; show an error if launch fails. Live entries still focus their window; URL-less history stays read-only. Browser tab/window policy and normal Project recovery apply. Debug build passes; live URL launching awaits user verification.
- [x] 8.11 — Show each restored window title and destination Project in the toast, retaining the different-Desktop indication. Page larger restoration batches in groups of five for six seconds each. Empty titles fall back to the app name. Debug build passes; app restarted.
...
- **Changing unrelated defaults or behaviour with Projects disabled.** Task 3.29 explicitly adds Desktop destination badges and Back navigation independently of Projects.
...
- **2026-09-08: show cross-Desktop window destinations and provide Back in the window switcher.** Back is a normal stop in the window cycle, and Command-Z performs a direct return; returning creates a reverse destination. This user request authorizes the focused switcher integration in task 3.29.

- **2026-09-08: make Space labels closable and explicitly created**, with managed Exposé participation. The user confirmed Exposé works and clarified half-screen width with compact height, width-fitted bold white text on black, one-time Bring to Front, click-away back ordering and native/bulk minimization. These supersede the initial fixed-size, persistent floating overlay design.

- **2026-09-08: add M3.5 between M3 and M4** for one always-on-top 500 × 200 px window per Space, showing its label and number in ordinary use and Mission Control / Exposé. Retain the plan's existing Projects opt-in guard; verify per-Space overview visibility before implementing the full lifecycle.
...
- **Projects are opt-in behind one `projectsEnabled` switch, off by default.** Desktop naming, icon choice/pinning, Space thumbnails, destination badges and Back navigation remain available independently.
...
- **Desktop changes follow the destination by default:** `projectsFollowDesktop` defaults on, selecting the linked Project or normal Desktop scope on Desktop changes and startup. Turning it off keeps a valid custom Project selected across Desktop changes and restores its saved selection at startup.
