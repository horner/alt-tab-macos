...
enum Projects {
    ...
    private static var initialSetupInProgress = false
    ...
    private static func initializeDesktopProjectsIfNeeded() {
        guard !initialSetupInProgress else { return }
        let desktops = spaces.map { ProjectsSetupResolver.Desktop(uuid: $0.uuid, hasLinkedProject: !linkedProjects(for: forSpace(uuid: $0.uuid)).isEmpty) }
        guard let uuids = ProjectsSetupResolver.desktopsToLink(enabled: isEnabled, completed: Preferences.projectsInitialSetupCompleted,
            storageReady: storageReady, desktops: desktops) else { return }
        initialSetupInProgress = true
        isLoading = true
        for uuid in uuids {
            let desktop = forSpace(uuid: uuid)
            guard let project = createCustom(homeSpaceUuid: uuid) else { continue }
            let name = ProjectNameResolver.normalized(desktop.name) ?? ProjectNameResolver.normalized(desktop.autoName) ?? project.resolvedName
            project.name = availableProjectName(name, excluding: project.id)
            project.autoName = desktop.autoName
            link(desktop, to: project)
        }
        isLoading = false
        ProjectPersistence.saveSnapshot { saved in
            if saved { Preferences.set("projectsInitialSetupCompleted", "true", false) }
