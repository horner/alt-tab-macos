...
enum ProjectPersistence {
    ...
    static func saveSnapshot(completion: @escaping (Bool) -> Void) {
        pendingSnapshot?.cancel()
        pendingSnapshot = nil
        Projects.persistSnapshot()
        // The serial queue checks the submitted snapshot's file writes before returning to main.
        queue.async {
            let saved = ready && store?.errors.isEmpty == true
