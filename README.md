...
On first launch, Projects is enabled, each Desktop gets a linked Project, and its existing windows become members as discovery completes. Existing Desktop names seed new Project names, with unique fallbacks for unnamed entries. Space labels open behind other windows and appear for 1,500 ms when switching Spaces. Saved names, memberships, exclusions and settings are preserved.

Take snapshots from Desktop labels, browse snapshot history from Project menus, and view Markdown summaries and window screenshots inside the app. Snapshot previews open on your current Desktop, including when reopening a closed or minimized preview. Projects and archives live in editable folders under `~/.alt-tab-projects`.

The ZIP contains `AltTabProjects.app`, signed with Developer ID and notarized by Apple. Both Apple silicon and Intel architectures are included. Packaging and Gatekeeper checks were tested on macOS 26.5.1 / Apple silicon; behavior on other Macs is still being tested.
